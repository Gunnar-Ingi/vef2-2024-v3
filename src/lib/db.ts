import pg from 'pg';
import { DatabaseTeam, Team } from '../types.js';
import {
  departmentsMapper,
  teamMapper,
  teamsMapper,
...
*/
  ...
  export async function getTeams(): Promise<Array<Team> | null>{
    ...
    const result = await query('SELECT id, name FROM teams');
    ...
    //const mapTeam = teamMapper(result);

    const teamsMap = teamsMapper(result.rows).map((t) => {
      return t;
    });
